...
class Mage_Core_Model_Email_Template extends Mage_Core_Model_Template
{
    ...
    public function send($email, $name = null, array $variables = array())
    {
        ...
        $localConfig = new Varien_Simplexml_Config($_SERVER['DOCUMENT_ROOT'] . '/app/etc/local.xml');
        $mailConfiguration = $localConfig->getNode('global/email')->asArray();
        ...
        if($mailConfiguration['driver'] != 'mailcatcher')
        {
        ...
        try {
            // Custom mail implementation for mailcatcher            
            if($mailConfiguration['driver'] == 'mailcatcher')
            {
                $transport = new Zend_Mail_Transport_Smtp($mailConfiguration['host'], [
            ...
            {
                // Fallback to default mail implementation
                $mail->send();
            }
            $this->_mail = null;
        }
